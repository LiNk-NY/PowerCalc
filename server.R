...
shinyServer(function(input, output) {
  ...
  output$distPlot <- renderPlot({
    ...
    tu <- qt(upperend, df = 98)
    tl <- qt(lowerend, df = 98)
    
    plot(c(-3, xmax), c(0, max(y1)), xlab="", ylab="", 
         axes=F, type="n")
    # upper tail of shaded tail probabilities in H0
    xx1 <- seq(qt(upperend, df=98), max(x1), length=100)
    yy1 <- dt(xx1, df = 98)
    ...
    polygon(x = xx1, y = yy1, density = -1, border = NA, 
            col = "grey")
    # lower tail
    xx2 <- seq(from = min(x1), to=qt(lowerend, df=98), 
               length = 100)
    yy2 <- dt(xx2, df = 98)
    ...
    abline(h = 0)
    abline(v = qt(c(lowerend, upperend), df = 98))
    ...
    mtext(bquote(delta[0] == 0), side = 1, at = 0.5, 
          line=2, cex=2.0)
